    ...
    public class JsonMapper
    {
        ...
        private static object ReadValue (Type inst_type, JsonReader reader, bool skipFirstRead = false)
        {
            ...
            if (reader.Token == JsonToken.ArrayStart) {
                ...
                // 为了应对待反序列化的对象是集合元素的情况：
                // 判断是否有PolymorphismJsonAttribute，
                // 若有，则根据记录的类型信息以子类类型确实例化对象；
                // 否则，直接实例化对象
                string property;
                PolymorphismJsonAttribute attr = GetPolymorphismJsonAttribute(value_type);
                if (attr != null)
                {
                ...
                while (true) {
                    ...
                    if (t_data.Properties.ContainsKey (property)) {
                        ...
                        if (prop_data.IsField) {
                            // 判断字段的类型是否有PolymorphismJsonAttribute，
                            // 若有，递归读取该字段；
                            // 否则，直接读取字段
                            if (HasPolymorphismJsonAttribute(prop_data.Type))
                            {
                                reader.Token = JsonToken.ObjectStart;
                            }
                            object v = ReadValue(prop_data.Type, reader);
                            ((FieldInfo)prop_data.Info).SetValue(instance, v);
                        } else {
                            PropertyInfo p_info =
        ...
        private static bool HasPolymorphismJsonAttribute(Type t)
        {
        ...
        private static PolymorphismJsonAttribute GetPolymorphismJsonAttribute(Type t)
        {
            PolymorphismJsonAttribute attr = Attribute.GetCustomAttribute(t, typeof(PolymorphismJsonAttribute), true) as PolymorphismJsonAttribute;
            if (attr != null) 
                return attr;

            foreach (var interfaceType in t.GetInterfaces())
            {
                attr = Attribute.GetCustomAttribute(t, typeof(PolymorphismJsonAttribute), true) as PolymorphismJsonAttribute;
                if (attr != null)
                    return attr;
            }
                                        ...
                                        int depth)
        {
            ...
            // 如果该类型有PolymorphismJsonAttribute，则记录其类型信息
            if (HasPolymorphismJsonAttribute(obj_type))
            {
